...
module Operational {
    ...
    public func createMaintenanceHandler(args: Arg, action: Actions<MaintenanceRecordCArg, MaintenanceRecordUArg>): async UpdateResult {
        ...
        let crudHandler: CrudHandler<K, C, U, T, StableT> = {
            ...
            mutate = func(arg: MaintenanceRecordUArg, maintenance: T): T {
                maintenance.description := Utils.get(arg.description, maintenance.description);
                maintenance.status := Utils.get(arg.status, maintenance.status);
                maintenance.dateCompleted := Utils.getNullable(arg.dateCompleted, maintenance.dateCompleted);
                maintenance.cost := Utils.getNullable(arg.cost, maintenance.cost);
                maintenance.contractor := Utils.getNullable(arg.contractor, maintenance.contractor);
                maintenance.paymentMethod := Utils.getNullable(arg.paymentMethod, maintenance.paymentMethod);
                maintenance.dateReported := Utils.getNullable(arg.dateReported, maintenance.dateReported);
                maintenance;
            };
      ...
      let handler = CrudHandler.generateGenericHandler<P, Nat, C, U, T, StableT, S>(
        ...
        CrudHandler.atomicActionToWhat(func(a: Types.Actions<C,U>): Types.What = #Maintenance(a))
      );
    ...
    public func createInspectionHandler(args: Arg, action: Actions<InspectionRecordCArg, InspectionRecordUArg>): async UpdateResult {
        ...
        let crudHandler: CrudHandler<K, C, U, T, StableT> = {
            ...
            i.inspectorName := Utils.get(arg.inspectorName, i.inspectorName);
            i.findings := Utils.get(arg.findings, i.findings);
            i.date := Utils.getNullable(arg.date, i.date);
            i.actionRequired := Utils.getNullable(arg.actionRequired, i.actionRequired);
            i.followUpDate := Utils.getNullable(arg.followUpDate, i.followUpDate);
            i;
        };
      ...
      let handler = CrudHandler.generateGenericHandler<P, Nat, C, U, T, StableT, S>(
        ...
        func(property: P){{property with operational = Stables.fromPartailStableOperationalInfo(operational)}},
        Handler.updatePropertyEventLog,
        CrudHandler.atomicActionToWhat(func(a: Types.Actions<C,U>): Types.What = #Inspection(a))
      );
      await Handler.applyHandler<P, K, A, T, StableT>(args, CrudHandler.makeAutomicAction(action, map.size()), handler);
    };
    ...
    public func createTenantHandler(args: Arg, action: Actions<TenantCArg, TenantUArg>): async UpdateResult {
        ...
        let crudHandler: CrudHandler<K, C, U, T, StableT> = {
            ...
            mutate = func(arg: TenantUArg, tenant: TenantUnstable): TenantUnstable {
                ...
                let updatedTenant = {
                    t with
                    leadTenant = Utils.get(arg.leadTenant, t.leadTenant);
                    otherTenants = Utils.get(arg.otherTenants, t.otherTenants);
                    monthlyRent = Utils.get(arg.monthlyRent, t.monthlyRent);
                    deposit = Utils.get(arg.deposit, t.deposit);
                    leaseStartDate = Utils.get(arg.leaseStartDate, t.leaseStartDate);
                    contractLength = Utils.get(arg.contractLength, t.contractLength);
                    paymentHistory = Utils.get(arg.paymentHistory, t.paymentHistory);
                    principal = Utils.getNullable(arg.principal, t.principal);
                };
        ...
        func(property: P){{property with operational = Stables.fromPartailStableOperationalInfo(operational)}},
        Handler.updatePropertyEventLog,
        CrudHandler.atomicActionToWhat(func(a: Types.Actions<C,U>): Types.What = #Tenant(a))
      );
      await Handler.applyHandler<P, K, A, T, StableT>(args, CrudHandler.makeAutomicAction(action, map.size()), handler);    
    };
