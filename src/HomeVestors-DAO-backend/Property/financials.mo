  ...
  public func createValuationHandler(args: Arg, action: Actions<ValuationRecordCArg, ValuationRecordUArg>): async UpdateResult {
    ...
    let crudHandler: CrudHandler<K, C, U, T, StableT> = {
      ...
      mutate = func(arg: U, v: T): T {
        v.value := Utils.get(arg.value, v.value);
        v.method := Utils.get(arg.method, v.method);
        v;
      };
    ...
    let handler = CrudHandler.generateGenericHandler<P, K, C, U, T, StableT, S>(
      ...
      CrudHandler.atomicActionToWhat(func(a: Types.Actions<C,U>): Types.What = #Valuations(a))
    );
