...
module {
    ...
    public type ActionToString = actor {
        actionToText: shared query Blob -> async Text;
    };

    public type PropertyModule = actor {
        updateProperty: shared (Blob, Blob) -> async Blob;
    };
    ...
    public type TokenActor = actor {
      ...
      icrc2_approve: shared TokenApproveArgs -> async TokenApproveResult;
    };
    ...
    public type NFTActor = actor {
        ...
        icrc37_approve_tokens: shared ([NFTApproveTokenArg]) -> async [?NFTApproveTokenResult];
    };
    ...
    public type ActionDictionary = actor {
        createProposal: shared (DAOState, ProposalArg, Principal, Principal) -> async Result.Result<DAOState, ()>;
        executeVote: shared (DAOState, VoteArgs, Principal, Principal) -> async Result.Result<DAOState,()>;
        executeProposal: (DAOState, Nat) -> async Result.Result<DAOState, ()>;
    };
