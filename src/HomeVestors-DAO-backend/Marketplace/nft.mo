...
module NFTCollections {
    ...
    public func getAllAccounts(canisterId: Principal): async ([(Account, Nat)], Nat){
        ...
        let map = HashMap.HashMap<Account, Nat>(0, Utils.accountEqual, Utils.accountHash);
    ...
    public func handleNFTMetadataUpdate(what: Types.AllWhats, property: Property): async () {
        let updates = switch (what) {
            ...
            case (#Valuations(_)) {
                switch(Utils.getElementByKey<Nat, Types.ValuationRecord>(property.financials.valuations, property.financials.valuationId, Nat.equal)){
                    case(null){return};
                    case(?v){
