...
module {
    ...
    public func createInvoiceHandler(args: Arg, action: Actions<InvoiceCArg, InvoiceUArg>): async UpdateResult {
            ...
            mutate = func(arg: U, el: T): T {
              switch(el.status){
                  case(#Draft){
                      ...
                      el.amount := Utils.get(arg.amount, el.amount);
                      el.due := Utils.get(arg.dueDate, el.due);
                      el.direction := Utils.get(arg.direction, el.direction);
                      el.paymentMethod := Utils.get(arg.paymentMethod, el.paymentMethod);
                      el.recurrence := Utils.get(arg.recurrence, el.recurrence);
                      el.status := switch(arg.preApprovedByAdmin, arg.process, Principal.equal(args.caller, Utils.getAdmin())){
        ...
        let transactionIds = HashMap.HashMap<Nat, AsyncType>(0, Nat.equal, Utils.natToHash);
        ...
        let handler: Handler<P, K, A, T, StableT> = {
                ...
                asyncEffect = func(arr: [(?K, A, Result.Result<T, UpdateError>)]): async [Result.Result<(), UpdateError>] {
                    if(args.testing) return Handler.runNoAsync<K, A, T>(arr);
                ...
                toStruct = CrudHandler.toStruct<P, K, C, U, T, StableT>(func(stableT: ?StableT) = #Invoice(stableT), func(property: Property) = property.financials.invoices, Nat.equal);
                ...
                toArgDomain = CrudHandler.atomicActionToWhat(func(a: Types.Actions<C,U>): Types.What = #Invoice(a));
            };
        ...
        switch(paymentStatus, cond){
            ...
            case(#Confirmed(paid), ?#Confirmed(arg)){
                switch(arg.paidFrom, arg.paidTo){
                    case(null, ?to) Utils.matchEqualityFlag(paid.paid_at, ?#LessThan(to));
                    case(?from, null) Utils.matchEqualityFlag(paid.paid_at, ?#MoreThan(from));
                    case(?from, ?to) Utils.matchEqualityFlag(paid.paid_at, ?#LessThan(to)) and Utils.matchEqualityFlag(paid.paid_at, ?#MoreThan(from));
                    case(null, null) true;
                };
            };
    ...
    public func filterInvoices(el: Invoice, conditional: Types.InvoiceConditionals): Types.ReadOutcome<Invoice> {
        ...
        if(
            ...
            Utils.matchEqualityFlag(el.amount, conditional.amount) and
            Utils.matchEqualityFlag(el.due, conditional.due) and
            Utils.matchEqualityFlag(endDate, conditional.recurrenceEndAt)
        ){
            #Ok(el);
        }
