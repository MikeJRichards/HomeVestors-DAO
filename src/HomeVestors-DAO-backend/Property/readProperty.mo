...
module Property {
    ...
    func houseValueWithinRanges(min: ?Nat, max: ?Nat, property: Property): Bool {
        let map = HashMap.fromIter<Nat, ValuationRecord>(property.financials.valuations.vals(), property.financials.valuations.size(), Nat.equal, Utils.natToHash);
    ...
    func returnElById<T>(property: Property, ids: [Result.Result<Nat, Nat>], elementIds: ?[Int], handler: ReadHandler<T>): [ElementResult<T>]{
        ...
        for(id in ids.vals()){
            switch(id){
                ...
                case(#ok(id)){
                    let value = switch(handler.filter, Utils.getElementByKey(arr, id, Nat.equal)){
    ...
    func returnPropertyElement<T>(arg: ReadArg, ids: ?[Int], handler: SimpleReadHandler<T>): [ElementResult<T>]{
        ...
        let configuredIds = Utils.convertIds(Iter.toArray(arg.properties.entries()), ids);
    ...
    func _filterNatArray<T>(el : (Nat, [(Nat, T)]), ids: ?[Int]): (Nat, [(Nat, T)]) {
        ...
        switch(ids){
            ...
            case(?ids){
                ...
                for(id in ids.vals()){
                    let idx = if(id > 0) id else arr.size() + id;
                    switch(Utils.getElementByKey(arr, Int.abs(idx), Nat.equal)){
                        case(null){}; 
    ...
    public func read<T>(args: ReadArg, reads: [Read2]): [ReadResult] {
    ...
    func getSpecificElements<T>(arg: ReadArg, propertyIds: ?[Int], ids: ?[Int], elementIds: ?[Int], handler: ReadHandler<T>): [PropertyResult<T>]{
        ...
        let results = Utils.convertIds<Property>(Iter.toArray(arg.properties.entries()), propertyIds);
                    ...
                    switch(arg.properties.get(propertyId)){
                        ...
                        case(?property){
                            if(filterProperty(property, arg.filterProperties)){
                                let convertedIds = Utils.convertIds(handler.toEl(property), ids);
    ...
    func matchAcc(listing: Listing, acc: ?Account, ltype: {#Seller; #Winning; #Purchased; #PreviousBids;}): Bool {
        ...
        Utils.matchNullableAccountArr(acc, accounts);
    };
}