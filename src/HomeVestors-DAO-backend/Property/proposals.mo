...
module {
    ...
    public func createProposalHandlers(arg: Arg, action: Actions<ProposalCArg, ProposalUArg>):async UpdateResult {
        ...
        let crudHandler : CrudHandler<K, C, U, T, StableT> = {
            ...
            mutate = func(arg: U, el: T): T {
                switch(el.status){
                    case(#LiveProposal(live)){
                        el.title := Utils.get(arg.title, el.title);
                        el.startAt := Utils.get(arg.startAt, el.startAt);
                        el.description := Utils.get(arg.description, el.description);
                        el.category := Utils.get(arg.category, el.category);
                        el.implementation := Utils.get(arg.implementation, el.implementation); //need to alter timer and end time - if this has changed
                        el.actions := Utils.get(arg.actions, el.actions);
                ...
                switch(proposal.status){
                    case(#LiveProposal(live)){
                        ...
                        switch(action){
                            ...
                            case(#Delete(_)){
                                if(not arg.testing){
                                    if(arg.caller != Utils.getAdmin() and live.endTime > Time.now()) return #err(#InvalidData{field = "End Time"; reason = #CannotBeSetInTheFuture;});
                                    if(proposal.eligibleVoters.size() == 0) return #err(#InvalidData{field = "eligible voters"; reason = #CannotBeNull;});
                                }
                            }; 
                        };
        ...
        let voters = HashMap.HashMap<Nat, [Principal]>(0, Nat.equal, Utils.natToHash);
        ...
        let handler: Handler<P, K, A, T, StableT> = {
                ...
                switch(action){
                    case(#Create(_)){
                                    ...
                                    for(accountOpt in accounts.vals()){
                                        let account = Utils.get(accountOpt, {owner = Utils.getAdmin(); subaccount = null});
                    ...
                    case(_) Handler.runNoAsync<K, A, T>(arr);
                }
            };
            ...
            toStruct = CrudHandler.toStruct<P, K, C, U, T, StableT>(func(stableT: ?StableT) = #Proposal(stableT), func(property: Property) = property.governance.proposals, Nat.equal);
            ...
            toArgDomain = CrudHandler.atomicActionToWhat(func(a: Types.Actions<C,U>): Types.What = #Governance(#Proposal(a)));
        };
    ...
    public func voteHandler(args: Arg, arg: Types.VoteArgs): async UpdateResult {
        ...
        let handler: Handler<P, K, A, T, StableT> = {
                  ...
                  case(?proposal){
                      switch(proposal.status){
                          case(#LiveProposal(live)){
                            ...
                            if(not Utils.isInList<Principal>(args.caller, proposal.eligibleVoters, Principal.equal) and not args.testing) return (?arg.proposalId, arg, #err(#InvalidData{field = "Eligible Voters"; reason = #InvalidInput}));
                            if(Utils.isInList<(Principal, Bool)>((args.caller, true), proposal.votes, func ((a, _), (b, _)) { Principal.equal(a, b) })) return (?arg.proposalId, arg, #err(#InvalidData{field = "Votes"; reason = #AlreadyVoted}));
            ...
            asyncEffect = func(arr: [(?K, A, Result.Result<T, UpdateError>)]): async [Result.Result<(), UpdateError>] { Handler.runNoAsync<K, A, T>(arr);};
            ...
            toStruct = func(property: Property, idOpt: ?Nat, beforeOrAfter: UnstableTypes.BeforeOrAfter): Types.ToStruct<K> {
                switch(idOpt){
                    case(?id) #Proposal(Utils.getElementByKey(property.governance.proposals, id, Nat.equal));
   ...
   func matchOutcome(el: Proposal, outcome: ?Types.ProposalOutcomeFlag): Bool {
    ...
    public func matchProposalConditions(el: Proposal, conditionals: Types.ProposalConditionals): Types.ReadOutcome<Proposal> {
        ...
        if(
            ...
            Utils.matchWhat(el.actions, conditionals.actions) and
            Utils.matchNullablePrincipals(?el.creator, conditionals.creator) and
            Utils.matchEqualityFlag(el.eligibleVoters.size(): Int, conditionals.eligibleCount) and
            Utils.matchEqualityFlag(el.votes.size(): Int, conditionals.totalVoterCount) and
            Utils.matchEqualityFlag(el.startAt, conditionals.startAt) and
            Utils.matchEqualityFlag(yesVotes, conditionals.yesVotes) and
            Utils.matchEqualityFlag(noVotes, conditionals.noVotes)
        ){
            #Ok(el);
        }
