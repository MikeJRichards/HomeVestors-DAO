...
import Prop "Property/property";
import Financial "Property/financials";
import NFT "Marketplace/nft";
import Tokens "Marketplace/token";
import UserNotifications "User/userNotifications";
import TestRouter "Property/Tests/testRouter";
import TestTypes "Property/Tests/testTypes";
...
import ReadProperty "Property/readProperty";
...
persistent actor {
  ...
  transient var globals : Types.Globals = {
    var propertyId = 0;
    var properties = HashMap.HashMap<Nat, Property>(0, Nat.equal, Utils.natToHash);
    var users = HashMap.HashMap<Principal, User>(0, Principal.equal, Principal.hash);
    var liquidations = HashMap.HashMap<Nat, Nat>(0, Nat.equal, Utils.natToHash);
  };

  var stableGlobals: Types.StableGlobals = {
    propertyId = 0;
    properties = [];
    users = [];
    liquidations = [];
  ...
  public func createProperty(nftCollection: Principal, nftQuantity: Nat): async [?MintResult] {
    let (property, mintResult) = await Prop.addProperty(globals.propertyId, nftCollection, nftQuantity);
    globals.properties.put(globals.propertyId, property);
    globals.propertyId += 1;
    return mintResult;
  };

  public func getProperty(id: Nat): async GetPropertyResult {
    return Utils.getPropertyFromId(id, globals.properties);
  };

  public func getAllProperties(): async [Property]{
    Iter.toArray(globals.properties.vals());
  };

  public func removeProperty(id: Nat): async Result.Result<Property, UpdateError>{
    Prop.removeProperty(id, globals.properties);
  };

  func handlePropertyUpdate(action: Types.AllWhatsWithPropertyId, caller: Principal): async Types.UpdateResultExternal {
    let parent = switch(globals.properties.get(action.propertyId)){case(?p) p; case(null) return #Err([(?action.propertyId, #InvalidPropertyId)])};
    switch(await Prop.updateProperty({what = action.what; caller; parent; handlePropertyUpdate; testing = false})){
      case(#Property(res)){
        globals.properties.put(action.propertyId, res.parent);
      ...
      case(#User(_)) return #Err([(null, #InvalidParent)])
    };
  };
  ...
  public query func readProperties(read: [Read2], filterProperties: ?FilterProperties): async [ReadResult] {
    ReadProperty.read({properties = globals.properties; filterProperties}, read);
  };
  ...
  public func getNFTs(ids: ?[Int], acc: Account): async ReadResult {
        let propertyIds = Utils.convertIds(Iter.toArray(globals.properties.entries()), ids);
        ...
        for(id in propertyIds.vals()){
          switch(id){
            case(#err(id)){};
            case(#ok(id)){
              switch(globals.properties.get(id)){
  ...
  public func emptyState(quantity: Nat): async [{nullRes: Nat; ok: Nat; err: [MintError]}]{
    let array = Iter.toArray(globals.properties.entries());
    let results = Buffer.Buffer<{nullRes: Nat; ok: Nat; err: [MintError]}>(globals.properties.size());
    globals.properties := HashMap.HashMap<Nat, Property>(0, Nat.equal, Utils.natToHash);
    globals.propertyId := 0;      
    for((_, property) in array.vals()){
      await NFT.clearNFTState(property.nftMarketplace.collectionId);
      let (newProperty, mintResult) = await Prop.addProperty(property.id, property.nftMarketplace.collectionId, quantity);
      let result = countResult(mintResult);
      if(result.ok > 0){
        globals.properties.put(property.id, newProperty);
        globals.propertyId += 1;
      };
      results.add(result);
    };
...
public func verifyNFTTransfer(propertyId: Nat, to: Account, token_id: Nat): async ?TransferResult {
    let property = switch(globals.properties.get(propertyId)){case(null)return null; case(?p)p};
    await NFT.verifyTransfer(property.nftMarketplace.collectionId, ?Principal.toBlob(Utils.getAdmin()), to, token_id);
  };
  ...
  public func verifyTokenTransferFrom(propertyId: Nat, listingId: Nat, from: Account): async ?TransferFromResult {
    let property = switch(globals.properties.get(propertyId)){case(null)return null; case(?property)property};
    switch(Utils.getElementByKey(property.nftMarketplace.listings, listingId, Nat.equal)){
  ...
  public func transferNFT(to: Account, listId: Nat, propertyId: Nat, tokenId:Nat): async ?TransferResult {
    let property = switch(globals.properties.get(propertyId)){case(?prop) prop; case(null){return ?#Err(#InvalidRecipient)}};
    switch(Utils.getElementByKey(property.nftMarketplace.listings, listId, Nat.equal)){
      case(null) return null;
      case(?#LaunchFixedPrice(arg)){
...
public func transferNFTBulk(): async [?TransferResult] {
    let property = switch(globals.properties.get(0)){case(?prop) prop; case(null){return [?#Err(#InvalidRecipient)]}};
    let buffer = Buffer.Buffer<TransferArg>(0);
    buffer.add(NFT.createTransferArg(2, null, {owner = Utils.getAdmin(); subaccount = ?Principal.toBlob(Utils.getAdmin())}));
    buffer.add(NFT.createTransferArg(3, null, {owner = Utils.getAdmin(); subaccount = ?Principal.toBlob(Utils.getAdmin())}));
    await NFT.transferBulk(property.nftMarketplace.collectionId, Buffer.toArray(buffer));
  };

  public func getBackendsNFTs(): async [Nat] {
    let property = switch(globals.properties.get(0)){case(?prop) prop; case(null){return []}};
  ...
  public func transferAllNFTs(): async [?TransferResult] {
    ...
    for(token in tokens.vals()){
      buffer.add(NFT.createTransferArg(token, null, {owner = Utils.getAdmin(); subaccount = ?Principal.toBlob(Utils.getAdmin())}));
    };
    await NFT.transferBulk(property.nftMarketplace.collectionId, Buffer.toArray(buffer));
  };
  ...
  public shared ({caller}) func updatePropertyValuations(): async [Types.UpdateResultExternal]{
    let results = Buffer.Buffer<Types.UpdateResultExternal>(globals.properties.size());
    for(property in globals.properties.vals()){
  ...
  public func getURLs():async [Text]{
    let buff = Buffer.Buffer<Text>(globals.properties.size());
    for(property in globals.properties.vals()){
  ...
  //  UserNotifications.getUserNotifications(account, globals.users);
  //};
//
...
//
  ...
  //  UserNotifications.getUserNotificationsResults(account, globals.users);
  //};
//
...
//
  ...
  //Stablecoin stuff
  ///////////////////
  //func handleUserUpdate(action: Types.AllStablecoinWhats, caller: Principal): async Types.UpdateResultExternal {
  //  let parent = switch(globals.users.get(caller)){case(?p) p; case(null) UserNotifications.createNewUser(caller)};
  //  switch(await Prop.updateProperty({what = action.what; caller; parent; handlePropertyUpdate; testing = false})){
  //    case(#Property(res)){
  ...
  //public shared ({caller}) func updateUser(action: WhatWithPropertyId): async Types.UpdateResultExternal {
  //  await handlePropertyUpdate(action, caller);
  //};
...
//
  ...
  //  properties := HashMap.fromIter(stableProperties.vals(), 0, Nat.equal, Utils.natToHash);
