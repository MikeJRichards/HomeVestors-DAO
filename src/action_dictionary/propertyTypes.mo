...
//    // Account Structure
    ...
    public type AllWhatsWithPropertyId = {
        propertyId: Nat;
        what: PropertyWhats;
    };
...
//HUSD
////////////////////
public type AssetKind = {
    #HUSD: FungibleAsset;
    #HVR: FungibleAsset;
    #NFT: NFTAsset;
  };

  public type FungibleAsset = {
    amount: Nat;
  };
  ...
  public type StabilityPool = {
    totalDepositedHUSD: Nat; //probably don't need this
    depositors: [Depositor]; //probably don't need this
    recentLiquidations: [LiquidationRecord]; //maybe make this a global variable
  };

  public type Depositor = {
    principal: Principal;
    stakedHUSD: Nat;
    stakeTime: Int;       // accumulated time since last reset
    lastUpdate: Int;      // timestamp of last update
  };

  public type LiquidationRecord = {
    propertyId: Nat;
    tokenId: Nat;
    nftValueUSD: Nat;
    winner: Principal;
  ...
  public type Globals = {
    var propertyId: Nat;
    var properties: HashMap.HashMap<Nat,Property>;
    var users: HashMap.HashMap<Principal, User>;
    var liquidations: HashMap.HashMap<Nat, Nat>;
  };

  public type StableGlobals = {
    propertyId: Nat;
    properties: [(Nat, Property)];
    users: [(Principal, User)];
    liquidations: [(Nat, Nat)];
  };
  ...
  public type User = {
    ...
    savedId: Nat;
    vault: Vault;
    stabilityPosition: StabilityPosition;
    transactionHistory: [(Nat, Transaction)];
    kyc: Bool;
    notifications : [(Nat, Notification)];
    saved: [(Nat, Nat)];
  };

  public type UserUnstable = {
    id: Principal;
    var transactionId: Nat;
    var notificationId: Nat;
    var savedId: Nat;
    var vault: Vault;
    var stabilityPosition: StabilityPosition;
    transactionHistory: HashMap.HashMap<Nat, Transaction>;
    var kyc: Bool;
    notifications: HashMap.HashMap<Nat, Notification>;
    saved: HashMap.HashMap<Nat, Nat>;
  };

  //-----------------------------
  // 💰 Stability Position (per user summary)
  //-----------------------------

  public type StabilityPosition = {
    depositedHUSD: Nat;
    pendingHVRRewards: Nat;
    pendingNFTRewards: [NFTAsset];
    lastReward: Int;
  };

  public type StablecoinAction<D, W> = {
    #Deposit: D;
    #Withdraw: W;
  };

    public type StablecoinWhat = {
        #HVRVault : StablecoinAction<AmountArg, AmountArg>; //deposit or withdraw hvr
        #NFTVault: StablecoinAction<NFTDepositArg, NFTDepositArg>; //deposit or withdraw nfts
        #ManageHUSDLoan: StablecoinAction<AmountArg, AmountArg>; //mint / burn
        #RedeemHUSD: StablecoinAction<AmountArg, AmountArg>; //singleton arg
        #HUSDStabilityPool: StablecoinAction<AmountArg, AmountArg>; //deposit or withdraw husd
        #ClaimNFT: NFTAsset;
        #ClaimHVR: Nat;
    };

    public type InternalStablecoinWhat = {
        #LiquidateVault: Principal;
        #CheckOvercollaterization;
    };

    public type UserWhat = {
        #Notification; 
        #KYC;
        #SavedProperty;
        #Transactions;
    ...
    public type InternalPropertyWhats = {
        #None;
    };
    public type PropertyWhats = What or InternalPropertyWhats;
    public type AllStablecoinWhats = StablecoinWhat or InternalStablecoinWhat;
    public type AllWhats = AllStablecoinWhats or PropertyWhats;

    public type AmountArg = { amount: Nat };
    public type NFTDepositArg = { propertyId: Nat; tokenIds: [Nat] };
